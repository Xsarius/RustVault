...
### 10.2 Brand Logos (Banks & Merchants)

**Strategy: Bundled defaults + lazy Brandfetch for the rest**

#### Bundled icon set (~80 inline SVGs, shipped with the app)

| Category | Examples |
|----------|----------|
| **Major banks (EU focus)** | Revolut, N26, Wise, ING, Santander, mBank, PKO BP, HSBC, Deutsche Bank, Commerzbank, BNP Paribas, Société Générale, Raiffeisen, UniCredit, Monzo, Starling, Lunar, Zen, Bunq, KBC |
| **Global banks** | Chase, Bank of America, Wells Fargo, Citibank, Barclays, UBS, Credit Suisse |
| **Payment services** | PayPal, Stripe, Apple Pay, Google Pay, Klarna, Afterpay |
| **Popular merchants** | Amazon, Spotify, Netflix, YouTube, Apple, Google, Facebook/Meta, Uber, Bolt, Allegro, Zalando, IKEA, Steam, GitHub |

#### Lazy loading via Brandfetch (on-demand)

For brands not in the bundled set, logos are fetched from the [Brandfetch API](https://brandfetch.com) on demand:

1. **Trigger** — when a bank is added or a transaction payee has no matching icon, the UI checks the bundled set first.
2. **Cache miss** — if not bundled, a request is made to Brandfetch by domain (e.g., `wise.com`). The free tier allows 25 requests/day, which is sufficient for incremental setup.
3. **Local cache** — fetched logos are stored locally (as SVG/PNG in the database or filesystem) so the same brand is never fetched twice.
4. **Offline safe** — after first fetch, logos are served from cache. No runtime dependency on Brandfetch.
5. **Fallback** — if Brandfetch is unavailable or rate-limited, display a colored circle with the brand's initial letter.

#### Resolution flow

```
Request logo for "brand-x"
  → Check bundled SVG set → ✓ Found → render inline SVG
  → ✗ Not bundled → check local cache → ✓ Found → render cached logo
  → ✗ Not cached → fetch from Brandfetch API → cache locally → render
  → ✗ Fetch failed → render colored initial avatar (fallback)
```

#### Implementation notes

- Store a `logo_source` field on Bank/Merchant: `bundled`, `brandfetch`, or `custom`.
- Brandfetch API key stored in server-side config (never exposed to client).
- Logo fetch happens server-side; client receives the cached asset.
- Users can always override with color picker + initial via the icon customizer.
